...
// Logs en memoria para PROPIETARIOS
let sentLogPropietariosSet = null;
let sentLogPropietariosEverSet = null;

// Horarios y configuración
const HORA_ENVIO_INQUILINOS = { hour: 12, minute: 59 };
const DIA_PILDORAS = 3; // 0=domingo, 3=miércoles
...
// Cargar log de PROPIETARIOS una vez y consultar duplicados en memoria
async function loadSentLogPropietarios() {
  const sheets = getSheetsClient();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SHEET_LOG_PROPIETARIOS}!A1:D10000`
  });
  ...
  sentLogPropietariosSet = set;
  sentLogPropietariosEverSet = setEver;
}

// Verificar si ya se envió alguna vez a PROPIETARIO
function alreadySentPropietarioEver(number, code) {
  if (!sentLogPropietariosEverSet) return false;
  const num = (number || '').toString().trim();
  const codeNorm = (code || '').toString().trim().toUpperCase();
  return sentLogPropietariosEverSet.has(`${num}|${codeNorm}`);
}

// Cachear log de PROPIETARIOS
let cacheLogPropietarios = null;
let cacheLogPropietariosEver = null;
let cacheLogPropietariosTS = 0;
// Usar CACHE_MS ya declarado globalmente
async function getCachedLogPropietarios() {
  const now = Date.now();
  if (!cacheLogPropietariosTS || now - cacheLogPropietariosTS > CACHE_MS) {
    await loadSentLogPropietarios();
    cacheLogPropietarios = sentLogPropietariosSet;
    cacheLogPropietariosEver = sentLogPropietariosEverSet;
    cacheLogPropietariosTS = now;
  } else {
    sentLogPropietariosSet = cacheLogPropietarios;
    sentLogPropietariosEverSet = cacheLogPropietariosEver;
  }
  return { daily: sentLogPropietariosSet, ever: sentLogPropietariosEverSet };
}
...
// Enviar todos los mensajes pendientes a PROPIETARIOS (incluyendo E-120, E-00, etc)
async function processPropietarioRow(client, row, templates) {
  const e164 = normalizePhoneE164(row.number);
  if (!e164) {
    console.log(`[PROPIETARIOS][SKIP] ${row.number}: número no válido o no normalizado`);
    return false;
  }
  const templateSet = templates.propietarios || {};
  const today = todayInTZ();
  const canSendNow = isAtOrAfterHourTZ(HORA_ENVIO_INQUILINOS.hour, HORA_ENVIO_INQUILINOS.minute);
  if (!canSendNow) {
    console.log(`[DEBUG][PROPIETARIOS] No es hora de enviar mensajes.`);
    return false;
  }
  await getCachedLogPropietarios();
  const jid = toWhatsAppJid(e164);
  let sentSomething = false;
  // Usar la misma lógica de getPendingMessages para obtener todos los códigos pendientes
  const pendientes = getPendingMessages({ ...row, address: 'PROPIETARIOS' }, templates);
  for (const { code, msg } of pendientes) {
    const isDup = alreadySentPropietarioEver(e164, code);
    console.log(`[DEBUG][PROPIETARIOS] Evaluando envío: número=${e164}, code=${code}, duplicado=${isDup}`);
    if (isDup) continue;
    await sendAndLogPropietarioMessage(client, jid, msg, code, e164, row);
    sentSomething = true;
    if (!sentLogPropietariosEverSet) sentLogPropietariosEverSet = new Set();
    sentLogPropietariosEverSet.add(`${e164}|${code}`);
    if (!sentLogPropietariosSet) sentLogPropietariosSet = new Set();
    const fecha = today.toISOString().slice(0, 10);
    sentLogPropietariosSet.add(`${e164}|${code}|${fecha}`);
  }
...
const SHEET_TEMPL_PROPIETARIOS  = 'PROPIETARIOS';
const SHEET_LOG_PROPIETARIOS    = 'LOG_PROPIETARIOS';
...
async function loadAllTemplates() {
  ...
  loadTemplatesFor(SHEET_TEMPL_PROPIETARIOS),
...
function pickTemplateSet(templates, address) {
  ...
  // Soporte para PROPIETARIOS
  if (addr.includes('PROPIETARIO')) return templates.propietarios || {};
...
function getPendingMessages(row, templates) {
  ...
  // E-XX: desde entrada hasta reserva (sin limitación de días)
  if (row.entry && today <= row.entry) {
    ...
    if (daysBetween(row.entry, today) === 0) {
      // Calcular días desde reserva hasta entrada para determinar el rango máximo
      const diasDesdeReserva = row.reserve ? daysBetween(row.entry, row.reserve) : 0;
      const maxDias = Math.max(diasDesdeReserva, 30); // Al menos 30, o más si hay más días desde reserva
      
      // Primero todos los E-XX desde el máximo hasta E-01
      ...
      // Comportamiento normal: todos los E-XX hasta hoy (sin limitación)
      const diasHastaEntrada = daysBetween(row.entry, today);
      const diasDesdeReserva = row.reserve ? daysBetween(row.entry, row.reserve) : diasHastaEntrada;
      const maxDias = Math.max(diasDesdeReserva, diasHastaEntrada, 30); // Sin limitación artificial
  ...
  // E+XX: desde el día después de entrada hasta el día antes de salida (sin limitación de días)
  ...
  // S-XX/S-00: desde entrada hasta salida (sin limitación de días)
  if (row.exit && today <= row.exit && row.entry && today >= row.entry) {
    const dToExitAbs = daysBetween(row.exit, today);
    if (dToExitAbs === 0 && set['S-00']) {
      pendientes.push({ code: 'S-00', msg: replaceTemplateVars(set['S-00'], row) });
    } else if (dToExitAbs > 0) {
  ...
  // S+XX: después de la salida (sin limitación de días)
...
async function processRRHHRow(client, row, templates) {
  ...
  if (!orderedTemplates.length) {
    console.log(`[DEBUG][PROPIETARIOS] No hay plantillas para propietarios.`);
    return false;
  }
  ...
  for (const { key, message } of orderedTemplates) {
    ...
    await sendAndLogPropietarioMessage(client, jid, rendered, code, e164, row);
    sentSomething = true;
    if (!sentLogPropietariosEverSet) {
      sentLogPropietariosEverSet = new Set();
    }
    sentLogPropietariosEverSet.add(`${e164}|${code}`);
    if (!sentLogPropietariosSet) {
      sentLogPropietariosSet = new Set();
    }
    const fecha = today.toISOString().slice(0, 10);
    sentLogPropietariosSet.add(`${e164}|${code}|${fecha}`);
  }
  ...
  console.log('✅ WhatsApp listo. Iniciando ciclo…');
    ...
    const tick = async () => {
      ...
      try {
        ...
        let messagesSent = 0;
        let rrhhMessagesSent = 0;
        for (const row of tenants) {
          if (!row.active) continue;
          activeTenants++;
          
          // Dividir direcciones por comas para procesar múltiples direcciones por fila
          const addresses = (row.address || '').split(',').map(addr => addr.trim()).filter(addr => addr);
          if (!addresses.length) continue; // Si no hay direcciones, saltar
          
          for (const address of addresses) {
            const addressNormalized = address.toUpperCase().replace(/\s+/g, '');
            if (addressNormalized === 'PILDORAS') continue; // saltar PILDORAS aquí

            // PROPIETARIOS
            if (addressNormalized === 'PROPIETARIOS') {
              let sent = await processPropietarioRow(client, row, TEMPLATES);
              if (sent) {
                anyMessage = true;
            ...
            if (addressNormalized.includes('RRHH')) {
              // RRHH usa exactamente la misma lógica que inquilinos (fechas de entrada/salida)
              await getCachedLogPildoras();
              const rowWithSingleAddress = { ...row, address: address };
              const e164 = normalizePhoneE164(row.number);
              if (!e164) {
                invalidNumbers++;
              ...
              const jid = toWhatsAppJid(e164);
              const pendientes = getPendingMessages(rowWithSingleAddress, TEMPLATES);
              pendingCandidates += pendientes.length;
              for (const { code, msg } of pendientes) {
                const dup = alreadySentPildoraEver(e164, code);
                if (dup) {
                  duplicatesSkipped++;
                  continue;
                }
                anyMessage = true;
                await sendAndLogMessage(client, jid, msg, code, e164, rowWithSingleAddress, true);
                rrhhMessagesSent++;
                await new Promise(res => setTimeout(res, 60000));
              }
              continue;
            ...
            for (const { code, msg } of pendientes) {
              ...
              anyMessage = true;
              await sendAndLogMessage(client, jid, msg, code, e164, rowWithSingleAddress);
              messagesSent++;
              await new Promise(res => setTimeout(res, 60000));
            }
          }
        }
        console.log(`[RESUMEN] Activos procesados: ${activeTenants}, mensajes detectados: ${pendingCandidates}, enviados en este ciclo: ${messagesSent + rrhhMessagesSent} (inquilinos: ${messagesSent}, RRHH: ${rrhhMessagesSent}), duplicados omitidos: ${duplicatesSkipped}, números inválidos: ${invalidNumbers}.`);
        // Calcular cuánto falta para la próxima ejecución (Europe/Madrid) usando HORA_ENVIO_INQUILINOS
        const now = new Date();
        const tz = TZ || 'Europe/Madrid';
        const targetHour = HORA_ENVIO_INQUILINOS.hour ?? 13;
        const targetMinute = HORA_ENVIO_INQUILINOS.minute ?? 0;
        const horaObjetivo = `${String(targetHour).padStart(2, '0')}:${String(targetMinute).padStart(2, '0')}`;
        const next = new Date(
        ...
        const horas = Math.max(0, Math.floor(msToNext / (1000 * 60 * 60)));
        const minutos = Math.max(0, Math.floor((msToNext % (1000 * 60 * 60)) / (1000 * 60)));
        if (!anyMessage) {
          console.log(`No hay mensajes para enviar hoy. Durmiendo hasta las ${horaObjetivo} (faltan ${horas}h ${minutos}m)...`);
        } else {
          console.log(`Todos los mensajes del día enviados. Durmiendo hasta las ${horaObjetivo} (faltan ${horas}h ${minutos}m)...`);
        }
        sleeping = true;
        setTimeout(() => {
          sleeping = false;
          console.log(`Reanudando ciclo tras dormir hasta las ${horaObjetivo}.`);
        }, Math.max(msToNext, 0));
      } catch (e) {
    ...
    const tickPildoras = async () => {
      try {
        ...
        for (const row of tenants) {
          ...
          // Dividir direcciones por comas para manejar múltiples direcciones
          const addresses = (row.address || '').split(',').map(addr => addr.trim()).filter(addr => addr);
          const hasPildoras = addresses.some(addr => addr.toUpperCase().replace(/\s+/g, '') === 'PILDORAS');
          
          if (!hasPildoras) continue; // Solo procesar si tiene PILDORAS en alguna dirección
        ...
        // Después de procesar todos los usuarios, dormir hasta el próximo miércoles
        const now = new Date();
        const tz = TZ;
        const horaObjetivo = `${String(HORA_ENVIO_PILDORAS.hour).padStart(2, '0')}:${String(HORA_ENVIO_PILDORAS.minute).padStart(2, '0')}`;
        ...
        next.setUTCDate(next.getUTCDate() + (7 - next.getUTCDay() + DIA_PILDORAS) % 7); // Próximo miércoles
        if (next.getUTCDate() === now.getUTCDate()) {
          // Si hoy es miércoles, programar para el próximo miércoles
          next.setUTCDate(next.getUTCDate() + 7);
        }
        
        const nextFormatted = new Date(
          new Intl.DateTimeFormat('en-CA', {
            timeZone: tz,
            year: 'numeric',
            month: '2-digit',
            day: '2-digit'
          }).format(next) + `T${horaObjetivo}:00`
        );
        
        const nowInTZ = new Date(now.toLocaleString('en-US', { timeZone: tz }));
        const msToNext = nextFormatted.getTime() - now.getTime();
        const dias = Math.max(0, Math.floor(msToNext / (1000 * 60 * 60 * 24)));
        const horas = Math.max(0, Math.floor((msToNext % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)));
        const minutos = Math.max(0, Math.floor((msToNext % (1000 * 60 * 60)) / (1000 * 60)));
        
        if (enviadosHoy > 0) {
          console.log(`[INFO] PILDORAS: Envíos del día completados (${enviadosHoy} píldoras enviadas de ${procesadosHoy} usuarios procesados). Durmiendo hasta el próximo miércoles a las ${horaObjetivo} (faltan ${dias}d ${horas}h ${minutos}m)...`);
        } else {
          console.log(`[INFO] PILDORAS: No hay píldoras para enviar hoy (${procesadosHoy} usuarios procesados, todos ya recibieron su píldora). Durmiendo hasta el próximo miércoles a las ${horaObjetivo} (faltan ${dias}d ${horas}h ${minutos}m)...`);
        }
        
        pildorasSleeping = true;
        setTimeout(() => {
          pildorasSleeping = false;
          console.log(`[INFO] PILDORAS: Reanudando ciclo para nuevo miércoles a las ${horaObjetivo}.`);
        }, Math.max(msToNext, 60000)); // Mínimo 1 minuto
